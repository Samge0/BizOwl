 ...
 */
function isCaptchaPage(document) {
...
function parseBaiduResults(context) {
    const accumulator = {
        results: [],
        seen: new Set()
    };
    ...
    const items = Array.from(root.querySelectorAll('.result, .c-container, [tpl], [srcid], #content_left > div[class]')).filter(el => {
        // 过滤掉广告、推荐等非搜索结果
    ...
    for (const item of items) {
        ...
        const rawUrl = item.getAttribute('mu')
            ...
            || '';
        // 兼容多版 snippet 选择器
        const snippetNode = item.querySelector('.c-abstract, .c-span-last, .result-summary, .c-color-text, [class*="abstract"], [class*="text"]');
        (0, shared_1.pushResult)(accumulator, types_1.SearchEngine.Baidu, titleNode?.textContent || '', (0, shared_1.parseUrl)(rawUrl, context.pageUrl), snippetNode?.textContent || item.textContent || '', context.maxResults);
    }
