  ...
  openclaw:
    ...
    requires:
      marketAuth:
        ...
        - IMA_OPENAPI_APIKEY
    primaryAuth: IMA_OPENAPI_CLIENTID
  security:
    credentials_usage: |
      ...
      Credentials are stored by BizOwl MCP Market in the hidden userData auth file
      mcp-installed/.auth/market-auth.json, and this skill reads only the ima entry from that file.
      ...
      No credentials are logged or transmitted to any other destination.
    allowed_domains:
      - ima.qq.com
      - '*.myqcloud.com'
---
...
IMA 凭证由 BizOwl MCP 市场的 **IMA 知识库与笔记** 卡片管理。

缺少凭证时，`node ima_api.cjs ...` 会以程序错误退出（`code: -100`），并在 stderr 输出对应 `msg`。收到缺少凭证错误时，引导用户：

1. 打开 https://ima.qq.com/agent-interface 获取 **Client ID** 和 **API Key**
2. 在 BizOwl 中打开 **MCP 市场 → IMA 知识库与笔记 → 编辑**
3. 填写 **Client ID** 和 **API Key** 后保存
...
if ! resp=$(node "$SKILL_DIR/ima_api.cjs" "openapi/list_docs" '{"limit":10}' 2>/tmp/ima_err); then
